...
public class TermiteBehaviour : MonoBehaviour
{
...
//	}
	...
	void FixedUpdate()
	{
		if (wandering)
		{
			...
			_controller.SimpleMove(forward * InputController.MoveSpeed);	
		}
		else
		{
			wandering = true;
...
//		float ceil = _heading + maxHeadingChange;
	...
	public void WoodFound(Transform wood)
	{
		if (hand.childCount == 0)
		{
			...
			wood.GetComponent<Collider>().enabled = false;
		...
		else
		{
			...
			w.GetComponent<Rigidbody>().useGravity = true;
